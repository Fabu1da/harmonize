...
class GPTConfidenceCalibrator:
    """
    Calibrates GPT-4 confidence scores using isotonic regression to align with true match accuracy.
    ...
    def collect_training_data(self, gpt_predictions: Dict, ground_truth: Dict):
        """
    ...
    def fit(self, min_samples=20):
        ...
        if len(self.calibration_data['raw_confidences']) < min_samples:
            print(f"⚠️ Insufficient data for GPT calibration: {len(self.calibration_data['raw_confidences'])} < {min_samples}")
            return False
        ...
        print(f"✅ GPT calibrator fitted on {len(X)} samples")
        return True
    
    def calibrate_confidence(self, raw_confidence: float) -> float:
        """
        Apply isotonic regression to calibrate a single GPT confidence score.
        
        Args:
            raw_confidence: Raw GPT-4 confidence score [0, 1]
    ...
    def calibrate_predictions(self, gpt_predictions: Dict) -> Dict:
        """
    ...
    def plot_calibration_curve(self, save_path: str = None):
        ...
        plt.plot(bin_centers, bin_accuracies, 'ro-', label='Raw GPT-4 Confidence')
        plt.plot([0, 1], [0, 1], 'k--', label='Perfect Calibration')
        plt.xlabel('Mean Predicted Confidence')
        plt.ylabel('Accuracy')
        plt.title('Raw GPT-4 Confidence Calibration')
        plt.legend()
        ...
        plt.plot(calibrated_bin_centers, calibrated_bin_accuracies, 'bo-', label='Calibrated GPT-4 Confidence')
        plt.plot([0, 1], [0, 1], 'k--', label='Perfect Calibration')
        plt.xlabel('Mean Predicted Confidence')
        plt.ylabel('Accuracy')
        plt.title('Calibrated GPT-4 Confidence')
        plt.legend()
        plt.grid(True, alpha=0.3)
...
\\begin{axis}[
    ...
    ylabel={Accuracy},
    title={GPT-4 Confidence Calibration},
...
"""
        ...
        latex_code += """};
\\addlegendentry{Raw GPT-4 Confidence}

% Calibrated GPT-4 confidence
...
"""
        ...
        latex_code += """};
\\addlegendentry{Calibrated GPT-4 Confidence}

\\end{axis}
\\end{tikzpicture}
\\caption{Reliability diagram showing GPT-4 confidence calibration before and after isotonic regression. Points closer to the diagonal indicate better calibration.}
...
"""
        ...
        with open("./output/gpt_calibration_plot.tex", "w") as f:
            f.write(latex_code)
        
        print("✅ LaTeX calibration plot code saved to ./output/gpt_calibration_plot.tex")
        return latex_code